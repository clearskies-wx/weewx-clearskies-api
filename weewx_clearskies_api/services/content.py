...
Expected filenames: about.md and legal.md
...
Security: filenames are hardcoded (no user input).  Path traversal defense is
belt-and-suspenders: os.path.realpath + containment assert even though the
filename cannot contain traversal sequences.
...
def read_content_file(filename: str) -> ContentFile | None:
    ...
    try:
        ...
        logger.warning(
            "Path traversal attempt detected for content file %r: "
